...
import { Sparkles, User, Mail, Lock, UserPlus, AlertCircle, Shield } from 'lucide-react';
    ...
    <div className="min-h-[85vh] bg-slate-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-3xl border border-slate-200 shadow-xl">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Create HirePrep AI Account</h2>
          <p className="text-xs text-slate-500">Join the platform to analyze resumes and practice AI interviews</p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-xl">
              <button
                ...
                className={`py-2 text-xs font-bold rounded-lg transition-all ${
                  role === 'CANDIDATE' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`py-2 text-xs font-bold rounded-lg transition-all ${
                  role === 'ADMIN' ? 'bg-white text-amber-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Platform Admin
              </button>
